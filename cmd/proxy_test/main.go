...
var (
	...
	protocol    = flag.String("protocol", "https", "代理协议")
...
func NewProxyTester(testURL string, verbose bool, min, max int) *ProxyTester {
	return &ProxyTester{
		testURL: testURL,
		min:     min,
		max:     max,
		current: int32(min),
		verbose: verbose,
		results: make(chan ProxyTestResult, min*3),
	}
}
...
func (pt *ProxyTester) TestProxy(ctx context.Context, proxyURL *url.URL, client *http.Client) ProxyTestResult {
	...
	//TCP 检测
	conn, err := net.DialTimeout("tcp", proxyURL.Host, time.Second*3)
	if err != nil {
		result.Error = fmt.Sprintf("tcp请求失败: %v", err)
...
func main() {
	...
	tester := NewProxyTester(*testURL, *verbose, actualConcurrency, actualConcurrency)

	// 创建工作池
	var wg sync.WaitGroup

	// 结果收集
	var results = make([]ProxyTestResult, 0, 1000000)
